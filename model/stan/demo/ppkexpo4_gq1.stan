...
//    array[nId] real CLNew, QNew, V2New, V3New, kaNew;
    vector[nId] CLNew, QNew, V2New, V3New, kaNew;
    ...
    for(m in 1:nSim){
      ...
      CLNew = exp(to_vector(thetaNew[,1]) + 0.75 * log(weight / 70) +
              EGFR_CL * log(EGFR / 90) +
              age_CL * log(age / 35) +
              albumin_CL * log(albumin / 4.5));
      QNew = exp(to_vector(thetaNew[,2]) + 0.75 * log(weight / 70));
      V2New = exp(to_vector(thetaNew[,3]) + log(weight / 70));
      V3New = exp(to_vector(thetaNew[,4]) + log(weight / 70));
      kaNew = exp(to_vector(thetaNew[,5]));
      for(j in 1:nId){
